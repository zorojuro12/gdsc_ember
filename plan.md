...
- [ ] 🟢 Create GitHub repo with `/frontend`, `/backend`, `/data` folder structure
- [ ] 🟢 Create `.gitignore` with: `node_modules/`, `__pycache__/`, `.env`, `*.pyc`, `dist/`, `.vite/`
- [ ] 🟢 Set up `.env.example` with `MAPBOX_TOKEN=`, `GOOGLE_MAPS_API_KEY=`, `ANTHROPIC_API_KEY=`, `DEMO_MODE=true`
- [ ] 🟢 Move scenario data files to `/data/scenarios/2023-west-kelowna/` — `mcdougall_creek_perimeter.geojson`, `shelters.json`, `road_closures.json`, `2023-08-15.csv` through `2023-08-19.csv`
- [ ] 🟢 Move docx files to `/docs/` — `EMBER_Project_Overview_Final_v2.docx`, `EMBER_Addendum_UI_API_Contracts.docx`
...
- [ ] 🟢 Evacuation zone overlays on Mapbox map: Order (red fill, 0.3 opacity), Alert (orange fill, 0.2 opacity), Watch (yellow fill, 0.15 opacity) — sourced from scenario data or placeholder polygons
- [ ] 🟢 Road closure layer: render lines from `road_closures.json` `coordinates_from`/`coordinates_to` as thick red dashed lines on map
- [ ] 🟢 Shelter pins layer: render markers from `shelters.json` lat/lng with color-coded status — green circle (Open), amber circle (Filling), red circle (Near Full)
- [ ] 🟢 Fire spread projection rings: render `spread_2hr.geojson`, `spread_4hr.geojson`, `spread_6hr.geojson` as translucent orange overlays with decreasing opacity (+2hr=0.3, +4hr=0.2, +6hr=0.1)
