...
const STATUS_STYLES: Record<string, string> = {
  ...
  ADVISORY: 'bg-yellow-900 text-yellow-300',
        ...
        <ul className="space-y-2">
          {roads.map((r, i) => (
            <li key={i} className="flex items-center justify-between gap-2">
              ...
              <span className={`text-[10px] font-bold px-2 py-0.5 rounded shrink-0 ${STATUS_STYLES[r.status] ?? 'bg-gray-800 text-gray-400'}`}>
                {r.status}
              </span>
            </li>
          ))}
